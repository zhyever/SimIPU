...
def create_groundtruth_database(dataset_class_name,
    ...
    if dataset_class_name == 'KittiDataset':
        ...
        dataset_cfg.update(
            ...
            load_interval=5, # do not need all
            pipeline=[
                dict(type='LoadImageFromFile'),
                dict(
                    type='LoadPointsFromFile',
    ...
    if database_save_path is None:
        database_save_path = osp.join(data_path, f'{info_prefix}_gt_database_fov')
    if db_info_save_path is None:
        db_info_save_path = osp.join(data_path,
                                     f'{info_prefix}_dbinfos_train_fov.pkl')
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        #######################  filter out img gt from side views
        # print("\n\n")
        # print(len(example['gt_bboxes']))
        # print(len(example['gt_bboxes_3d']))
        bbox_location = example['gt_bboxes_3d'].center.numpy()

        rect = np.array(example['rect']).reshape(4, 4)
        Trv2c = np.array(example['Trv2c']).reshape(4, 4)
        ...
        # print(example['gt_bboxes'].shape)
        # print(indices.shape)
        example['gt_bboxes'] = example['gt_bboxes'][indices.reshape([-1])]
        annos['gt_bboxes_3d'] = LiDARInstance3DBoxes(annos['gt_bboxes_3d'].tensor[indices.reshape([-1])])
        # print(annos['gt_names'].shape)
        annos['gt_names'] = annos['gt_names'][indices.reshape([-1])]
        # print(annos['gt_names'].shape)
        if 'group_ids' in annos:
            print("test1")
            print(annos['group_ids'])
            annos['group_ids'] = annos['group_ids'][indices.reshape([-1])]
            print(annos['group_ids'])
        if 'difficulty' in annos:
            print("test2")
            print(annos['difficulty'])
            annos['difficulty'] = annos['difficulty'][indices.reshape([-1])]
            print(annos['difficulty'])
        if 'score' in annos:
            print("test3")
            print(annos['score'])
            annos['score'] = annos['score'][indices.reshape([-1])]
            print(annos['score'])
        # hack: without mask. No need to change img shape
        # exit(10000000000)
        #######################
        ...
        for i in range(num_obj):
            ...
            rel_filepath = osp.join(f'{info_prefix}_gt_database_fov', filename)
