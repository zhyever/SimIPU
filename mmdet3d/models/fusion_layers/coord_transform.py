...
def apply_3d_transformation(pcd, coords_type, img_meta, reverse=False, points_center=None):
    ...
    if points_center != None:
        center = torch.tensor(points_center, dtype=dtype, device=device)
        neg_center = torch.tensor([-i for i in points_center], dtype=dtype, device=device)
        translate_tocenter = partial(pcd.translate, trans_vector=neg_center)
        translate_back = partial(pcd.translate, trans_vector=center)
        # reverse must be true, so it will be T2 R T1. T2 to center first
        flow_mapping = {
        'T': translate_func,
        'T1': translate_back,
        'T2': translate_tocenter,
        'S': scale_func,
        'R': rotate_func,
