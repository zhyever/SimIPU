...
class WaymoDataset(KittiDataset):
    ...
    def evaluate(self,
        ...
        if 'kitti' in metric:
            ...
            # hack:
            # mmcv.dump(result_files, osp.join(out_dir, "kitti_format.pkl"))
            # tmp_dir = None
            # result_files = mmcv.load(osp.join(out_dir, "kitti_format.pkl"))
            # print("load successfully")
            
            from mmdet3d.core.evaluation import kitti_eval
            gt_annos = [info['annos'] for info in self.data_infos]
            gt_annos_filted = []
            gt_annos_filted_double = []
            
            for item in gt_annos:
                mask = copy.deepcopy(item['camera_id'] == 0)
                # difficulty = copy.deepcopy(item['difficulty'] != -1)

                temp = {}
                for key, val in item.items():
                    temp[key] = val[mask]
                gt_annos_filted.append(temp)
            
            for item in gt_annos_filted:
                difficulty = copy.deepcopy(item['difficulty'] == 0)

                temp = {}
                for key, val in item.items():
                    temp[key] = val[difficulty]
                gt_annos_filted_double.append(temp)
            
            gt_annos = gt_annos_filted_double
        ...
        if 'waymo' in metric:
            ...
            result_files, tmp_dir = self.format_results(
                ...
                submission_prefix,
                data_format='waymo')
            # mmcv.dump(result_files, osp.join(out_dir, "waymo_format.pkl"))
            import subprocess
            ret_bytes = subprocess.check_output(
                'mmdet3d/core/evaluation/waymo_utils/' +
                f'compute_detection_metrics_main {pklfile_prefix}.bin ' +
                f'{waymo_root}/gt_fov.bin',
                shell=True)
