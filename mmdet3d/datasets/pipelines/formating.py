...
class Collect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                            ...
                            'pts_filename', 'transformation_3d_flow', 'P0')): # add waymo P0 here
        self.keys = keys
...
class DefaultFormatBundle3D(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        #############################################
        # Format 3D data    
        if 'points_ori' in results:
            assert isinstance(results['points_ori'], BasePoints)
            results['points_ori'] = DC(results['points_ori'].tensor)

        # 
        # # Format 3D data
        # if 'points_key' in results:
        #     assert isinstance(results['points_key'], BasePoints)
        #     results['points_key'] = DC(results['points_key'].tensor)
        ##############################################
        ...
        for key in [
                ...
                'attr_labels', 'centers2d', 'depths',
                
                ##### add key below
                'gt_bboxes_key', 'gt_bboxes_ignore_key', 'gt_labels_key',
                'gt_labels_3d_key'
        ]:
            if key not in results:
                continue
        ...
        #
        # if 'gt_bboxes_3d_key' in results:
        #     if isinstance(results['gt_bboxes_3d_key'], BaseInstance3DBoxes):
        #         results['gt_bboxes_3d_key'] = DC(
        #             results['gt_bboxes_3d_key'], cpu_only=True)
        #     else:
        #         results['gt_bboxes_3d_key'] = DC(
        #             to_tensor(results['gt_bboxes_3d_key']))
        ################################################      
