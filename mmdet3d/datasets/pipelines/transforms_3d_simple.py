...
class RandomFlip3DSimple(RandomFlip):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(RandomFlip3DSimple, self).__init__(
    ...
    def __call__(self, input_dict):
        ...
        super(RandomFlip3DSimple, self).__call__(input_dict)
...
@PIPELINES.register_module()
class ObjectSampleSimple(object):
...
@PIPELINES.register_module()
class ObjectNoiseSimple(object):
...
@PIPELINES.register_module()
class GlobalAlignmentSimple(object):
...
@PIPELINES.register_module()
class GlobalRotScaleTransSimple(object):
    ...
    def __init__(self,
                 ...
                 points_center=None, # for point-center transfer
                 shift_height=False):
        ...
        self.points_center = points_center
        self.neg_points_center = [-i for i in self.points_center]
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        if self.points_center != None:
            # trans to zero center
            input_dict['points'].translate(self.neg_points_center)

            # if no bbox in input_dict, only rotate points
            if len(input_dict['bbox3d_fields']) == 0:
                rot_mat_T = input_dict['points'].rotate(noise_rotation)
                input_dict['pcd_rotation'] = rot_mat_T
                # back to origin (early stage)
                input_dict['points'].translate(self.points_center)
                return
    ...
    def __call__(self, input_dict):
        ...
        # ## save for contrastive learning
        # input_dict['points_ori'] = copy.deepcopy(input_dict['points'])
        ...
        if self.points_center != None:
            input_dict['transformation_3d_flow'].extend(['T1', 'R', 'T2', 'S', 'T'])
        else:
...
@PIPELINES.register_module()
class PointShuffleSimple(object):
...
@PIPELINES.register_module()
class ObjectFovFilterSimple(object):
    """Filter objects by the range. (and fov)
    ...
    def __call__(self, input_dict):
        ...
        bbox_location = input_dict['gt_bboxes_3d'].center.numpy()

        rect = np.array(input_dict['rect']).reshape(4, 4)
        Trv2c = np.array(input_dict['Trv2c']).reshape(4, 4)
        P0 = np.array(input_dict['P0']).reshape(4, 4)

        indices = box_np_ops.remove_outside_points_mask(bbox_location, rect, Trv2c, P0, input_dict['ori_shape'])

        gt_bboxes_3d_filted = gt_bboxes_3d[indices.reshape([-1])]
        gt_bboxes_3d_filted = LiDARInstance3DBoxes(gt_bboxes_3d_filted)
        input_dict['gt_bboxes_3d'] = gt_bboxes_3d_filted
        indices = np.array(indices)
        gt_labels_3d = gt_labels_3d[indices.squeeze().astype(np.bool)]
        input_dict['gt_labels_3d'] = gt_labels_3d

        return input_dict

@PIPELINES.register_module()
class ObjectRangeFilterSimple(object):
...
@PIPELINES.register_module()
class PointsRangeFilterSimple(object):
    ...
    def __call__(self, input_dict):
        ...
        # # for contrastive leraning
        # points_ori = input_dict['points_ori']
        # points_mask_ori = points_ori.in_range_3d(self.pcd_range)
        # clean_points_ori = points_ori[points_mask_ori]
        # input_dict['points_ori'] = clean_points_ori
...
@PIPELINES.register_module()
class ObjectNameFilterSimple(object):
...
@PIPELINES.register_module()
class IndoorPointSampleSimple(object):
    """Indoor point sample.
    ...
    def __call__(self, results):
        ...
        """
        # # for contrastive
        # points_ori = results['points_ori']
        # points_ori, choices_ori = self.points_random_sampling(
        #     points_ori, self.num_points, return_choices=True)
        # results['points_ori'] = points_ori
        # ago
...
@PIPELINES.register_module()
class IndoorPatchPointSampleSimple(object):
...
@PIPELINES.register_module()
class BackgroundPointsFilterSimple(object):
...
@PIPELINES.register_module()
class VoxelBasedPointSamplerSimple(object):
