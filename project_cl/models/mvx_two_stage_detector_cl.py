...
from mmdet3d.ops.furthest_point_sample import Points_Sampler


@DETECTORS.register_module()
class MVXTwoStageDetector_cl(Base3DDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 pretraining=None):
        super(MVXTwoStageDetector_cl, self).__init__()

        self.pretraining = pretraining
        # print(train_cfg)
        if pretraining:
            self.cl_strategy = train_cfg.cl_strategy
            if self.cl_strategy.num_points != None:
                self.SA_modules = nn.ModuleList()
                
                if isinstance(self.cl_strategy.fps_mods, tuple):
                    cur_fps_mod = list(self.cl_strategy.fps_mods)
                else:
                    cur_fps_mod = list([self.cl_strategy.fps_mods])
                

                if isinstance(self.cl_strategy.fps_sample_range_lists, tuple):
                    cur_fps_sample_range_lists = list(self.cl_strategy.fps_sample_range_lists)
                else:
                    cur_fps_sample_range_lists = self.cl_strategy.fps_sample_range_lists
    
                self.point_sampler = Points_Sampler(self.cl_strategy.num_points, cur_fps_mod, cur_fps_sample_range_lists)
    ...
    def init_weights(self, pretrained=None):
        ...
        super(MVXTwoStageDetector_cl, self).init_weights(pretrained)
    ...
    def extract_img_feat(self, img, img_metas):
        ...
        if self.pretraining != True:
            ...
            if self.with_img_neck:
                img_feats = self.img_neck(img_feats)
                return img_feats, img_contrastive
        return img_feats
